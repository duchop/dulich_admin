...
class Swift_Encoder_Rfc2231EncoderTest extends \SwiftMailerTestCase
{
    ...
    public function testEncodingAsciiCharactersProducesValidToken()
    {
        ...
        foreach (explode("\r\n", $encoded) as $line) {
            $this->assertRegExp($this->rfc2045Token, $line, '%s: Encoder should always return a valid RFC 2045 token.');
        }
    }
    ...
    public function testMaximumLineLengthCanBeSet()
    {
        ...
        $this->assertEquals(str_repeat('a', 75) . "\r\n" . str_repeat('a', 75) . "\r\n" . str_repeat('a', 50), $encoded, '%s: Lines should be wrapped at each 75 characters');
    }
