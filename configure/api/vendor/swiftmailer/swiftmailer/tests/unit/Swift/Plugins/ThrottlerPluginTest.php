...
class Swift_Plugins_ThrottlerPluginTest extends \SwiftMailerTestCase
{
    ...
    public function testBytesPerMinuteThrottling()
    {
        ...
        // 10MB/min
        $plugin = new Swift_Plugins_ThrottlerPlugin(10000000, Swift_Plugins_ThrottlerPlugin::BYTES_PER_MINUTE, $sleeper, $timer);
        
        $timer->shouldReceive('getTimestamp')
            ->once()
            ->andReturn(0);
        $timer->shouldReceive('getTimestamp')
            ->once()
