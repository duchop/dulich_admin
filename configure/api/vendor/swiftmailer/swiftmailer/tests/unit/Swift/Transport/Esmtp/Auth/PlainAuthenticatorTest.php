...
class Swift_Transport_Esmtp_Auth_PlainAuthenticatorTest extends \SwiftMailerTestCase
{
    ...
    public function testSuccessfulAuthentication()
    {
        ...
        $this->agent->shouldReceive('executeCommand')
            ->once()
            ->with('AUTH PLAIN ' . base64_encode('jack' . chr(0) . 'jack' . chr(0) . 'pass') . "\r\n", [
            235
        ]);
        
        $this->assertTrue($plain->authenticate($this->agent, 'jack', 'pass'), '%s: The buffer accepted all commands authentication should succeed');
    }
