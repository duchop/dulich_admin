...
class Swift_Transport_Esmtp_Auth_NTLMAuthenticatorTest extends \SwiftMailerTestCase
{
    ...
    public function testGetDomainAndUsername()
    {
        ...
        $login = $this->getAuthenticator();
        list ($domain, $user) = $this->invokePrivateMethod('getDomainAndUsername', $login, [
            $username
        ]);
    ...
    public function testSuccessfulAuthentication()
    {
        ...
        $agent->shouldReceive('executeCommand')
            ->once()
            ->with('AUTH NTLM ' . base64_encode($this->invokePrivateMethod('createMessage1', $ntlm)) . "\r\n", [
            334
        ])
        ...
        $agent->shouldReceive('executeCommand')
            ->once()
            ->with(base64_encode($this->invokePrivateMethod('createMessage3', $ntlm, [
            $domain,
            $username,
