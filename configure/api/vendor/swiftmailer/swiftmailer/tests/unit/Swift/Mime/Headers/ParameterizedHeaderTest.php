...
class Swift_Mime_Headers_ParameterizedHeaderTest extends \SwiftMailerTestCase
{
    ...
    {
        $header = $this->getHeader('Content-Type', $this->getHeaderEncoder('Q', true), $this->getParameterEncoder(true));
    ...
    {
        $header = $this->getHeader('Content-Disposition', $this->getHeaderEncoder('Q', true), $this->getParameterEncoder(true));
        $header->setValue('attachment');
        $header->setParameters([
            'filename' => 'my file.txt'
        ]);
        $this->assertEquals('attachment; filename="my file.txt"', $header->getFieldBody());
    }
    ...
    public function testLongParamsAreBrokenIntoMultipleAttributeStrings()
    {
        ...
        $header = $this->getHeader('Content-Disposition', $this->getHeaderEncoder('Q', true), $encoder);
        $header->setValue('attachment');
        $header->setParameters([
            'filename' => $value
        ]);
        $header->setMaxLineLength(78);
        $this->assertEquals('attachment; ' . 'filename*0*=utf-8\'\'' . str_repeat('a', 63) . ";\r\n " . 'filename*1*=' . str_repeat('a', 63) . ";\r\n " . 'filename*2*=' . str_repeat('a', 54), $header->getFieldBody());
    }
    ...
    public function testEncodedParamDataIncludesCharsetAndLanguage()
    {
        ...
        $this->assertEquals('attachment; filename*=' . $this->charset . "'" . $this->lang . "'" . str_repeat('a', 20) . '%8F' . str_repeat('a', 10), $header->getFieldBody());
    }
    ...
    public function testMultipleEncodedParamLinesAreFormattedCorrectly()
    {
        ...
        $header->setMaxLineLength(78);
        $header->setLanguage($this->lang);
        $this->assertEquals('attachment; filename*0*=' . $this->charset . "'" . $this->lang . "'" . str_repeat('a', 20) . '%8F' . str_repeat('a', 28) . ";\r\n " . 'filename*1*=' . str_repeat('a', 32), $header->getFieldBody());
    }
