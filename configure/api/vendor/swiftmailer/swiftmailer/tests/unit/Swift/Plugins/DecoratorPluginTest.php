...
class Swift_Plugins_DecoratorPluginTest extends \SwiftMailerTestCase
{
    ...
    {
        $message = $this->createMessage($this->createHeaders(), [
            'zip@button.tld' => 'Zipathon'
        ], [
            'chris.corbyn@swiftmailer.org' => 'Chris'
        ], 'Subject', 'Hello {name}, you are customer #{id}');
    ...
    public function testReplacementsCanBeMadeInHeaders()
    {
        ...
        $message = $this->createMessage($headers, [
            ...
            'chris.corbyn@swiftmailer.org' => 'Chris'
        ], 'A message for {name}!', 'Hello {name}, you are customer #{id}');
        ...
        $returnPathHeader->shouldReceive('setFieldBodyModel')->zeroOrMoreTimes();
    ...
    {
        $message = (new Swift_Message('subject foo'))->setBody('body foo')
    ...
    private function createMessage($headers, $to = [], $from = null, $subject = null, $body = null)
    {
