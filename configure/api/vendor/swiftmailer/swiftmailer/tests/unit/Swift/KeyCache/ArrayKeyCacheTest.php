...
class Swift_KeyCache_ArrayKeyCacheTest extends \PHPUnit\Framework\TestCase
{
    ...
    public function testStringDataCanBeSetAndFetched()
    {
        ...
        $cache->setString($this->key1, 'foo', 'test', Swift_KeyCache::MODE_WRITE);
    ...
    public function testStringDataCanBeOverwritten()
    {
        ...
        $cache = $this->createCache($is);
        $cache->setString($this->key1, 'foo', 'test', Swift_KeyCache::MODE_WRITE);
        $cache->setString($this->key1, 'foo', 'whatever', Swift_KeyCache::MODE_WRITE);
    ...
    public function testStringDataCanBeAppended()
    {
        ...
        $cache = $this->createCache($is);
        $cache->setString($this->key1, 'foo', 'test', Swift_KeyCache::MODE_WRITE);
        $cache->setString($this->key1, 'foo', 'ing', Swift_KeyCache::MODE_APPEND);
    ...
    public function testNsKeyIsWellPartitioned()
    {
        ...
        $cache = $this->createCache($is);
        $cache->setString($this->key1, 'foo', 'test', Swift_KeyCache::MODE_WRITE);
        $cache->setString($this->key2, 'foo', 'ing', Swift_KeyCache::MODE_WRITE);
    ...
    public function testItemKeyIsWellPartitioned()
    {
        ...
        $cache = $this->createCache($is);
        $cache->setString($this->key1, 'foo', 'test', Swift_KeyCache::MODE_WRITE);
        $cache->setString($this->key1, 'bar', 'ing', Swift_KeyCache::MODE_WRITE);
    ...
    public function testByteStreamCanBeImported()
    {
        ...
        $is = $this->createKeyCacheInputStream();
        $cache = $this->createCache($is);
        $cache->importFromByteStream($this->key1, 'foo', $os, Swift_KeyCache::MODE_WRITE);
    ...
    public function testByteStreamCanBeAppended()
    {
        ...
        $cache->importFromByteStream($this->key1, 'foo', $os1, Swift_KeyCache::MODE_APPEND);
        $cache->importFromByteStream($this->key1, 'foo', $os2, Swift_KeyCache::MODE_APPEND);
    ...
    public function testByteStreamAndStringCanBeAppended()
    {
        ...
        $cache->setString($this->key1, 'foo', 'test', Swift_KeyCache::MODE_APPEND);
        $cache->importFromByteStream($this->key1, 'foo', $os, Swift_KeyCache::MODE_APPEND);
    ...
    public function testNsKeyCanBeCleared()
    {
        ...
        $cache->setString($this->key1, 'foo', 'test', Swift_KeyCache::MODE_WRITE);
        $cache->setString($this->key1, 'bar', 'xyz', Swift_KeyCache::MODE_WRITE);
        $this->assertTrue($cache->hasKey($this->key1, 'foo'));
