...
class Swift_ByteStream_ArrayByteStreamTest extends \PHPUnit\Framework\TestCase
{
    ...
    public function testReadingSingleBytesFromBaseInput()
    {
        ...
        $this->assertEquals($input, $output, '%s: Bytes read from stream should be the same as bytes in constructor');
    }
    ...
    public function testReadingMultipleBytesFromBaseInput()
    {
        ...
        $this->assertEquals([
            'ab',
            'cd'
        ], $output, '%s: Bytes read from stream should be in pairs');
    }
    ...
    public function testReadingOddOffsetOnLastByte()
    {
        ...
        $this->assertEquals([
            ...
            'cd',
            'e'
        ], $output, '%s: Bytes read from stream should be in pairs except final read');
    }
    ...
    public function testSettingPointerPartway()
    {
        ...
        $bs = $this->createArrayStream($input);
        $bs->setReadPointer(1);
        $this->assertEquals('b', $bs->read(1), '%s: Byte should be second byte since pointer as at offset 1');
    }
    ...
    public function testResettingPointerAfterExhaustion()
    {
        ...
        $bs->setReadPointer(0);
        $this->assertEquals('a', $bs->read(1), '%s: Byte should be first byte since pointer as at offset 0');
    }
    ...
    public function testPointerNeverSetsBelowZero()
    {
        ...
        $bs->setReadPointer(- 1);
        $this->assertEquals('a', $bs->read(1), '%s: Byte should be first byte since pointer should be at offset 0');
    }
    ...
    public function testPointerNeverSetsAboveStackSize()
    {
        ...
        $bs->setReadPointer(3);
        $this->assertFalse($bs->read(1), '%s: Stream should be at end and thus return false');
    }
    ...
    public function testBytesCanBeWrittenToStream()
    {
        ...
        $this->assertEquals([
            ...
            'd',
            'e'
        ], $output, '%s: Bytes read from stream should be from initial stack + written');
    }
    ...
    public function testContentsCanBeFlushed()
    {
        ...
        $this->assertFalse($bs->read(1), '%s: Contents have been flushed so read() should return false');
    }
    ...
    public function testConstructorCanTakeStringArgument()
    {
        ...
        $this->assertEquals([
            ...
            'b',
            'c'
        ], $output, '%s: Bytes read from stream should be the same as bytes in constructor');
    }
