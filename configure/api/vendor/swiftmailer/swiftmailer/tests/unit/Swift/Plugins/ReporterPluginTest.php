...
class Swift_Plugins_ReporterPluginTest extends \SwiftMailerTestCase
{
    ...
    public function testReportingPasses()
    {
        ...
        $evt->shouldReceive('getMessage')
            ->zeroOrMoreTimes()
            ->andReturn($message);
        $evt->shouldReceive('getFailedRecipients')
            ->zeroOrMoreTimes()
            ->andReturn([]);
        $reporter->shouldReceive('notify')
            ->once()
            ->with($message, 'foo@bar.tld', Swift_Plugins_Reporter::RESULT_PASS);
    ...
    public function testReportingFailedCc()
    {
        ...
        $reporter->shouldReceive('notify')
            ->once()
            ->with($message, 'test@test.com', Swift_Plugins_Reporter::RESULT_PASS);
