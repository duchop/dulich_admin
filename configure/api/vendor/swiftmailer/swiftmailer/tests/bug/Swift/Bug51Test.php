...
class Swift_Bug51Test extends \SwiftMailerTestCase
{
    ...
    public function testAttachmentsDoNotGetTruncatedUsingToByteStream()
    {
        ...
        for ($i = 0; $i < 10; ++ $i) {
            $message = $this->createMessageWithRandomAttachment(10000, $this->attachmentFile);
            
            file_put_contents($this->outputFile, '');
            $message->toByteStream(new Swift_ByteStream_FileByteStream($this->outputFile, true));
            
            $emailSource = file_get_contents($this->outputFile);
            
            $this->assertAttachmentFromSourceMatches(file_get_contents($this->attachmentFile), $emailSource);
        }
    }
    ...
    public function assertAttachmentFromSourceMatches($attachmentData, $source)
    {
        ...
        if (false === $attachmentDataEnd) {
            ...
            $attachmentBase64 = trim(substr($source, $attachmentDataStart, $attachmentDataEnd - $attachmentDataStart));
        }
    ...
    private function createMessageWithRandomAttachment($size, $attachmentPath)
    {
        ...
        $message = (new Swift_Message())->setSubject('test')
            ->setBody('test')
            ->setFrom('a@b.c')
            ->setTo('d@e.f')
