...
class ShellInput extends StringInput
{
    ...
    private function tokenize($input)
    {
        ...
        $cursor = 0;
        while ($cursor < $length) {
            if (\preg_match('/\s+/A', $input, $match, null, $cursor)) {} elseif (\preg_match('/([^="\'\s]+?)(=?)(' . StringInput::REGEX_QUOTED_STRING . '+)/A', $input, $match, null, $cursor)) {
                $tokens[] = [
                    $match[1] . $match[2] . \stripcslashes(\str_replace([
                        '"\'',
                        '\'"',
                        '\'\'',
                        '""'
    ...
    private function addShortOption($shortcut, $value)
    {
        ...
        $this->addLongOption($this->definition->getOptionForShortcut($shortcut)
            ->getName(), $value);
    }
    ...
    private function addLongOption($name, $value)
    {
        ...
        if (\in_array($value, [
            ...
            $nextToken = $next[0];
            if ((isset($nextToken[0]) && '-' !== $nextToken[0]) || \in_array($nextToken, [
                '',
                null
            ], true)) {
                $value = $nextToken;
