...
class Configuration
{
    ...
    public function __construct(array $config = [])
    {
        ...
        if (isset($config['baseDir'])) {
            $msg = "The 'baseDir' configuration option is deprecated; " . "please specify 'configDir' and 'dataDir' options instead";
    ...
    public function getConfigFile()
    {
        ...
        $files = ConfigPaths::getConfigFiles([
            'config.php',
            'rc.php'
        ], $this->configDir);
    ...
    public function getReadline()
    {
        if (! isset($this->readline)) {
            ...
            $this->readline = new $className($this->getHistoryFile(), $this->getHistorySize(), $this->getEraseDuplicates());
        }
    ...
    public function getOutput()
    {
        if (! isset($this->output)) {
            $this->output = new ShellOutput(ShellOutput::VERBOSITY_NORMAL, $this->getOutputDecorated(), null, $this->getPager());
        }
    ...
    /**
     * Get the decoration (i.e.
     * color) setting for the Shell Output service.
     *
    ...
    /**
     * Internal method for adding tab completion matchers.
     * This will set any new
    ...
    /**
     * Internal method for adding commands.
     * This will set any new commands once
    ...
    public function getManualDbFile()
    {
        ...
        $files = ConfigPaths::getDataFiles([
            'php_manual.sqlite'
        ], $this->dataDir);
        if (! empty($files)) {
            if ($this->warnOnMultipleConfigs && \count($files) > 1) {
