...
class Shell extends Application
{
    ...
    private function inputInOpenStringOrComment($input)
    {
        ...
        return $last === '"' || $last === '`' || (\is_array($last) && \in_array($last[0], [
     ...
     *
     * @param bool $includeBoundObject
     *            Pass false to exclude 'this'. If you're
    ...
    public function formatException(\Exception $e)
    {
        ...
        $message = \preg_replace("#(\\w:)?(/\\w+)*/src/Execution(?:Loop)?Closure.php\(\d+\) : eval\(\)'d code#", "eval()'d code", \str_replace('\\', '/', $message));
