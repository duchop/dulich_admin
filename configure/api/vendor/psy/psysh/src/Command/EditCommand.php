...
class EditCommand extends Command implements ContextAware
{
    ...
    protected function configure()
    {
        $this->setName('edit')
            ...
            new InputOption('exec', 'e', InputOption::VALUE_NONE, 'Execute the file content after editing. This is the default when a file name argument is not given.', null),
            new InputOption('no-exec', 'E', InputOption::VALUE_NONE, 'Do not execute the file content after editing. This is the default when a file name argument is given.', null)
        ])
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        $execute = $this->shouldExecuteFile($input->getOption('exec'), $input->getOption('no-exec'), $filePath);
    ...
    private function extractFilePath($fileArgument)
    {
        ...
        if ($fileArgument !== null && \strlen($fileArgument) > 0 && $fileArgument[0] === '$') {
    ...
    private function editFile($filePath, $shouldRemoveFile)
    {
        ...
        $pipes = [];
        $proc = \proc_open((\getenv('EDITOR') ?  : 'nano') . " {$escapedFilePath}", [
            STDIN,
            STDOUT,
            STDERR
        ], $pipes);
