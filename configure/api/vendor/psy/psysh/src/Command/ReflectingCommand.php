...
abstract class ReflectingCommand extends Command implements ContextAware
{
    ...
    protected function getTarget($valueName)
    {
        ...
        switch (true) {
            case \preg_match(self::CLASS_OR_FUNC, $valueName, $matches):
                return [
                    $this->resolveName($matches[0], true),
                    null,
                    0
                ];
            ...
            case \preg_match(self::CLASS_MEMBER, $valueName, $matches):
                return [
                    $this->resolveName($matches[1]),
                    $matches[2],
                    Mirror::CONSTANT | Mirror::METHOD
                ];
            ...
            case \preg_match(self::CLASS_STATIC, $valueName, $matches):
                return [
                    $this->resolveName($matches[1]),
                    $matches[2],
                    Mirror::STATIC_PROPERTY | Mirror::PROPERTY
                ];
     ...
     * execution context.
     * This is used to inject magic $__class, $__method and
