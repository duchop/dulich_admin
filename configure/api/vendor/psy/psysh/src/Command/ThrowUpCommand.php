...
HELP
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        $throwCode = $this->printer->prettyPrint([
            $throwStmt
        ]);
    ...
    private function prepareArgs($code = null)
    {
        ...
        $args = [
            new Arg($expr, false, false, $node->getAttributes())
        ];
        
        // Allow throwing via a string, e.g. `throw-up "SUP"`
        if ($expr instanceof String_) {
            return [
                new New_(new FullyQualifiedName('Exception'), $args)
            ];
        }
