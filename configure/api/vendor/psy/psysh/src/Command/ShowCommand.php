...
HELP
    ...
    private function writeExceptionContext(InputInterface $input, OutputInterface $output)
    {
        ...
        $output->writeln($this->getApplication()
    ...
    private function writeTraceLine(OutputInterface $output, array $trace, $index)
    {
        ...
        $output->writeln(\sprintf('From <info>%s:%d</info> at <strong>level %d</strong> of backtrace (of %d).', OutputFormatter::escape($file), OutputFormatter::escape($line), $index + 1, \count($trace)));
    }
    ...
    private function writeTraceCodeSnippet(OutputInterface $output, array $trace, $index)
    {
        ...
        $output->write($this->getHighlighter()
            ->getCodeSnippet($code, $line, 5, 5), ShellOutput::OUTPUT_RAW);
    }
