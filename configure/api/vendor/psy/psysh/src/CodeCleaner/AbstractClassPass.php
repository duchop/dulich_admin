    ...
    public function leaveNode(Node $node)
    {
        if ($node instanceof Class_) {
            ...
            if ($count > 0 && ! $node->isAbstract()) {
                $msg = \sprintf('Class %s contains %d abstract method%s must therefore be declared abstract or implement the remaining methods (%s)', $node->name, $count, ($count === 1) ? '' : 's', \implode(', ', $this->abstractMethods));
                throw new FatalErrorException($msg, 0, E_ERROR, null, $node->getLine());
            }
        }
    }
