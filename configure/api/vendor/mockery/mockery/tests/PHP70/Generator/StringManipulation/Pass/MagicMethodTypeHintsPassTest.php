...
class MagicMethodTypeHintsPassTest extends TestCase
{
    ...
    {
        $targetClass = DefinedTargetClass::factory('Mockery\Test\Generator\StringManipulation\Pass\MagicInterfaceDummy');
        $magicMethods = $this->pass->getMagicMethods($targetClass);
    ...
    public function itShouldAddStringTypeHintOnMagicMethod()
    {
        ...
        $code = $this->pass->apply('public function __isset($name) {}', $this->mockedConfiguration);
    ...
    public function itShouldAddStringTypeHintOnAllMagicMethods()
    {
        ...
        $code = $this->pass->apply('public function __isset($name) {}', $this->mockedConfiguration);
        $this->assertContains('string $name', $code);
        $code = $this->pass->apply('public function __unset($name) {}', $this->mockedConfiguration);
        $this->assertContains('string $name', $code);
    }
    ...
    {
        $this->configureForClass();
        $code = $this->pass->apply('public function __toString() {}', $this->mockedConfiguration);
    ...
    {
        $this->configureForClass();
        $code = $this->pass->apply('public function __call($method, array $args) {}', $this->mockedConfiguration);
    ...
    {
        $this->configureForClass();
        $code = $this->pass->apply('public static function __callStatic($method, array $args) {}', $this->mockedConfiguration);
        $this->assertContains('string $method', $code);
     ...
     * Mock class.
     * This is made to avoid variable
    ...
    protected function configureForClass(string $className = 'Mockery\Test\Generator\StringManipulation\Pass\MagicDummy')
    {
        ...
        $this->mockedConfiguration->shouldReceive('getTargetClass')
            ->andReturn($targetClass)
            ->byDefault();
        $this->mockedConfiguration->shouldReceive('getTargetInterfaces')
            ->andReturn([])
            ->byDefault();
    }
