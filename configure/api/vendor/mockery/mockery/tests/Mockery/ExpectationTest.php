...
class ExpectationTest extends MockeryTestCase
{
    ...
    {
        $this->mock->shouldReceive('foo')->andReturnValues(array(
            1,
            2,
            3
        ));
    ...
    {
        $exp = $this->mock->shouldReceive('foo')->with(array(
            'Spam' => 'Ham',
            'Bar' => 'Baz',
            'Bar',
            'Baz',
    ...
    public function testDucktypeConstraintNonMatchingCase()
    {
        $this->mock->shouldReceive('foo')->times(3);
        $this->mock->shouldReceive('foo')
            ->with(1, Mockery::ducktype('quack', 'swim'))
            ->never();
        $this->mock->foo();
        $this->mock->foo(1);
        $this->mock->foo(1, 2, 3);
    ...
    {
        $this->mock->shouldReceive('foo')->with(Mockery::subset(array(
            'a' => 1,
            'b' => 2
        )));
        $this->mock->foo(array(
    ...
    public function testHasKeyConstraintNonMatchingCase()
    {
        $this->mock->shouldReceive('foo')->times(3);
        $this->mock->shouldReceive('foo')
            ->with(1, Mockery::hasKey('a'))
            ->never();
        $this->mock->foo();
        $this->mock->foo(1);
        $this->mock->foo(1, array(
            'a' => 1
        ), 3);
    }
    ...
    public function testShouldIgnoreMissingDefaultSelfAndReturnsSelf()
    {
        ...
        $this->assertSame($this->mock, $this->mock->a()
            ->b());
    }
    ...
    public function testAnExampleWithSomeExpectationAmends()
    {
        ...
        $service->shouldReceive('addBookmark')
            ->with(Mockery::pattern('/^http:/'), \Mockery::type('string'))
            ->times(3)
            ->andReturn(true);
        $service->shouldReceive('hasBookmarksTagged')
            ->with('php')
    ...
    public function testIfExceptionIndicatesAbsenceOfMethodAndExpectationsOnMock()
    {
        ...
        $this->expectException('\BadMethodCallException', 'Method ' . get_class($mock) . '::nonExistent() does not exist on this mock object');
    ...
    public function testIfCallingMethodWithNoExpectationsHasSpecificExceptionMessage()
    {
        ...
        $this->expectException('\BadMethodCallException', 'Received ' . get_class($mock) . '::quack(), ' . 'but no expectations were specified');
    ...
    public function testWetherMockWithInterfaceOnlyCanNotImplementNonExistingMethods()
    {
        ...
        $waterMock->shouldReceive('nonExistentMethod')
            ->once()
            ->andReturnNull();
        \Mockery::close();
    }
    ...
    public function testCountWithBecauseExceptionMessage()
    {
        ...
        $this->expectExceptionMessageRegexp('/Method foo\(<Any Arguments>\) from Mockery_[\d]+ should be called' . PHP_EOL . ' ' . 'exactly 1 times but called 0 times. Because We like foo/');
    ...
    public function it_uses_a_matchers_to_string_method_in_the_exception_output()
    {
        ...
        $this->expectException(InvalidCountException::class, "Method foo(<HasKey[foo]>)");
