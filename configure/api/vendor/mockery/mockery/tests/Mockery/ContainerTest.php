...
class ContainerTest extends MockeryTestCase
{
    ...
    public function testGetKeyOfDemeterMockShouldReturnKeyWhenMatchingMock()
    {
        ...
        $this->assertRegExp('/Mockery_(\d+)__demeter_([0-9a-f]+)_foo/', Mockery::getContainer()->getKeyOfDemeterMockFor('foo', get_class($m)));
    }
    ...
    {
        $m = mock('stdClass, ArrayAccess, Countable', array(
            'foo' => 1,
            'bar' => 2
        ));
        $this->assertEquals(1, $m->foo());
    ...
    {
        $m = mock("MockeryTest_ClassConstructor2[foo]", array(
            $param1 = new stdClass()
        ));
    ...
    {
        $m = mock("MockeryTest_ClassConstructor2", array(
            $param1 = new stdClass()
        ));
        $m->makePartial();
        $this->assertEquals($param1, $m->getParam1());
    ...
    {
        $m = mock(new MockeryTestBar1(), array(
            'foo' => 1,
            Mockery\Container::BLOCKS => array(
                'method1'
            )
        ));
    ...
    {
        $file = mock('SplFileInfo[getFilename,getPathname,getExtension,getMTime]', array(
            __FILE__
        ));
    ...
    public function testInstantiationOfInstanceMockWithConstructorParameterValidationException()
    {
        ...
        $m->shouldReceive('__construct')->andThrow(new \Exception('instanceMock ' . rand(100, 999)));
    ...
    public function testMethodParamsPassedByReferenceHaveReferencePreserved()
    {
        ...
        $m->shouldReceive('foo')->with(Mockery::on(function (&$a) {
            $a += 1;
            return true;
        }), Mockery::any());
        $a = 1;
    ...
    {
        Mockery::getConfiguration()->setInternalClassMethodParamMap('DateTime', 'modify', array(
            '&$string'
        ));
        ...
        $m->shouldReceive('modify')->with(Mockery::on(function (&$string) {
            $string = 'foo';
            return true;
        }));
        $data = 'bar';
    ...
    public function testCanOverrideExpectedParametersOfExtensionPHPClassesToPreserveRefs()
    {
        ...
        Mockery::getConfiguration()->setInternalClassMethodParamMap('MongoCollection', 'insert', array(
            '&$data',
            '$options'
        ));
        // @ used to avoid E_STRICT for incompatible signature
    ...
    {
        $m = Mockery::mock('StdClass')->shouldReceive('get')
            ->andReturn(false)
            ->getMock();
        $m->get();
        Mockery::close();
    ...
    public function testHandlesMethodWithArgumentExpectationWhenCalledWithResource()
    {
        ...
        $mock->shouldReceive('foo')->with(array(
            'yourself' => 21
        ));
    ...
    public function testExceptionOutputMakesBooleansLookLikeBooleans()
    {
        ...
        $this->expectException("Mockery\Exception\NoMatchingExpectationException", "MyTestClass::foo(true, false, [0 => true, 1 => false])");
