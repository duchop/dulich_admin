...
abstract class TestCase extends Assert implements Test, SelfDescribing
{
    ...
    {
        return \PHPUnit\Util\Test::parseTestMethodAnnotations(\get_class($this), $this->name);
    }
    ...
    {
        return \PHPUnit\Util\Test::getSize(\get_class($this), $this->getName(false));
    }
    ...
    {
        try {
            $expectedException = \PHPUnit\Util\Test::getExpectedException(\get_class($this), $this->name);
    ...
    {
        try {
            $useErrorHandler = \PHPUnit\Util\Test::getErrorHandlerSettings(\get_class($this), $this->name);
    ...
    protected function checkRequirements()
    {
        ...
        $missingRequirements = \PHPUnit\Util\Test::getMissingRequirements(\get_class($this), $this->name);
    ...
    public function run(TestResult $result = null)
    {
        ...
        if (! $this instanceof WarningTestCase && ! $this instanceof SkippedTestCase && ! $this->handleDependencies()) {
            return $result;
        }
        
        $runEntireClass = $this->runClassInSeparateProcess && ! $this->runTestInSeparateProcess;
        
        if (($this->runTestInSeparateProcess === true || $this->runClassInSeparateProcess === true) && $this->inIsolation !== true && ! $this instanceof PhptTestCase) {
            $class = new ReflectionClass($this);
            
            if ($runEntireClass) {
                $template = new Text_Template(__DIR__ . '/../Util/PHP/Template/TestCaseClass.tpl');
            } else {
    ...
    {
        if ($this->name === null) {
            throw new Exception('PHPUnit\Framework\TestCase::$name must not be null.');
        }
    ...
    protected function getMockFromWsdl($wsdlFile, $originalClassName = '', $mockClassName = '', array $methods = [], $callOriginalConstructor = true, array $options = [])
    {
        ...
        $mockObject = $this->getMockObjectGenerator()->getMock($originalClassName, $methods, [
            '',
            $options
        ...
        if (! empty($this->dependencies) && ! $this->inIsolation) {
            ...
            foreach ($this->dependencies as $dependency) {
                ...
                if (! isset($passedKeys[$dependency])) {
                    ...
                    $this->result->addError($this, new SkippedTestError(\sprintf('This test depends on "%s" to pass.', $dependency)), 0);
                ...
                if (isset($passed[$dependency])) {
                    if ($passed[$dependency]['size'] != \PHPUnit\Util\Test::UNKNOWN && $this->getSize() != \PHPUnit\Util\Test::UNKNOWN && $passed[$dependency]['size'] > $this->getSize()) {
                        $this->result->addError($this, new SkippedTestError('This test depends on a test that is larger than itself.'), 0);
    ...
    private function stopOutputBuffering()
    {
        if (\ob_get_level() != $this->outputBufferingLevel) {
            ...
            throw new RiskyTestError('Test code or tested code did not (only) close its own output buffers');
        }
        ...
        if ($this->outputCallback === false) {
            ...
            $this->output = \call_user_func_array($this->outputCallback, [
                $output
            ]);
        }
    ...
    {
        if ($this->runTestInSeparateProcess || $this->inIsolation || (! $this->backupGlobals === true && ! $this->backupStaticAttributes)) {
            return;
        }
    ...
    private function createGlobalStateSnapshot($backupGlobals)
    {
        ...
        return new Snapshot($blacklist, $backupGlobals, (bool) $this->backupStaticAttributes, false, false, false, false, false, false, false);
    }
    ...
    private function checkExceptionExpectations(Throwable $throwable): bool
    {
        ...
        if (\is_string($this->expectedException)) {
            ...
            if ($this->expectedException === 'PHPUnit\Framework\Exception' || $this->expectedException === '\PHPUnit\Framework\Exception' || $reflector->isSubclassOf('PHPUnit\Framework\Exception')) {
                $result = true;
            }
        }
