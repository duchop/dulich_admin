...
class TestSuite implements Test, SelfDescribing, IteratorAggregate
{
    ...
    public function __construct($theClass = '', $name = '')
    {
        ...
        if (! $theClass->isSubclassOf(TestCase::class)) {
            throw new Exception('Class "' . $theClass->name . '" does not extend PHPUnit\Framework\TestCase.');
        }
    ...
    public static function createTest(ReflectionClass $theClass, $name)
    {
        ...
        $backupSettings = \PHPUnit\Util\Test::getBackupSettings($className, $name);
        
        $preserveGlobalState = \PHPUnit\Util\Test::getPreserveGlobalStateSettings($className, $name);
        
        $runTestInSeparateProcess = \PHPUnit\Util\Test::getProcessIsolationSettings($className, $name);
        
        $runClassInSeparateProcess = \PHPUnit\Util\Test::getClassProcessIsolationSettings($className, $name);
        ...
        if ($constructor !== null) {
            ...
            if (\count($parameters) < 2) {
                ...
                try {
                    $data = \PHPUnit\Util\Test::getProvidedData($className, $name);
                ...
                if (isset($data)) {
                    $test = new DataProviderTestSuite($className . '::' . $name);
                    ...
                    if ($data instanceof WarningTestCase || $data instanceof SkippedTestCase || $data instanceof IncompleteTestCase) {
    ...
    protected function addTestMethod(ReflectionClass $class, ReflectionMethod $method)
    {
        ...
        if ($test instanceof TestCase || $test instanceof DataProviderTestSuite) {
            $test->setDependencies(\PHPUnit\Util\Test::getDependencies($class->getName(), $name));
        }
        
        $this->addTest($test, \PHPUnit\Util\Test::getGroups($class->getName(), $name));
    }
