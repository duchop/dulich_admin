...
class PhptTestCase implements Test, SelfDescribing
{
    ...
    private function parse()
    {
        ...
        foreach ($unsupportedSections as $section) {
            if (isset($sections[$section])) {
                throw new Exception('PHPUnit does not support this PHPT file');
            }
        }
        ...
        foreach ($allowSections as $section) {
            if (isset($sections[$section . '_EXTERNAL'])) {
                ...
                if (! \is_file($testDirectory . $externalFilename) || ! \is_readable($testDirectory . $externalFilename)) {
                    throw new Exception(\sprintf('Could not load --%s-- %s for PHPT file', $section . '_EXTERNAL', $testDirectory . $externalFilename));
                }
    ...
    private function renderForCoverage(&$settings)
    {
        ...
        $template = new Text_Template(__DIR__ . '/../Util/PHP/Template/PhptTestCase.tpl');
        ...
        $template->setVar([
            ...
            'autoPrependFile' => \var_export(! empty($settings['auto_prepend_file']) ? $settings['auto_prepend_file'] : false, true)
        ]);
