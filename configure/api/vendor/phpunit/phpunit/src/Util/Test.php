...
class Test
{
    ...
    public static function getProvidedData($className, $methodName)
    {
        ...
        if ($data !== null) {
            foreach ($data as $key => $value) {
                if (! \is_array($value)) {
                    throw new Exception(\sprintf('Data set %s is invalid.', \is_int($key) ? '#' . $key : '"' . $key . '"'));
                }
            }
        }
        ...
        if (\preg_match(self::REGEX_TEST_WITH, $docComment, $matches, PREG_OFFSET_CAPTURE)) {
            ...
            foreach (\explode("\n", $annotationContent) as $candidateRow) {
                ...
                if (\json_last_error() != JSON_ERROR_NONE) {
                    throw new Exception('The dataset for the @testWith annotation cannot be parsed: ' . \json_last_error_msg());
                }
    ...
    public static function getSize($className, $methodName)
    {
        ...
        if (isset($groups['large']) || (\class_exists('PHPUnit\DbUnit\TestCase', false) && $class->isSubclassOf('PHPUnit\DbUnit\TestCase'))) {
            return self::LARGE;
        }
            ...
            if (isset($methodName[0]) && $methodName[0] == '<') {
                ...
                foreach ($classes as $className) {
                    if (! \class_exists($className) && ! \interface_exists($className) && ! \trait_exists($className)) {
                        throw new InvalidCoversTargetException(\sprintf('Trying to @cover or @use not existing class or ' . 'interface "%s".', $className));
                    }
                ...
                foreach ($classes as $className) {
                    if ($className == '' && \function_exists($methodName)) {
                        ...
                        if (! ((\class_exists($className) || \interface_exists($className) || \trait_exists($className)) && \method_exists($className, $methodName))) {
                            throw new InvalidCoversTargetException(\sprintf('Trying to @cover or @use not existing method "%s::%s".', $className, $methodName));
                        }
    ...
    private static function resolveReflectionObjectsToLines(array $reflectors)
    {
        ...
        foreach ($reflectors as $reflector) {
            ...
            $result[$filename] = \array_merge($result[$filename], \range($reflector->getStartLine(), $reflector->getEndLine()));
        }
