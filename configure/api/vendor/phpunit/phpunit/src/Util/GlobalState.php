...
class GlobalState
{
    ...
    public static function processIncludedFilesAsString(array $files)
    {
        ...
        for ($i = \count($files) - 1; $i > 0; $i --) {
            ...
            if (! empty($GLOBALS['__PHPUNIT_ISOLATION_BLACKLIST']) && \in_array($file, $GLOBALS['__PHPUNIT_ISOLATION_BLACKLIST'])) {
                continue;
            }
    ...
    {
        if (\is_scalar($variable) || null === $variable || (\is_array($variable) && self::arrayOnlyContainsScalars($variable))) {
