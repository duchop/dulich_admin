...
class Command
{
    ...
    {
        try {
            $this->options = Getopt::getopt($argv, 'd:c:hv', \array_keys($this->longOptions));
        } catch (Exception $t) {
        ...
        foreach ($this->options[0] as $option) {
            switch ($option[0]) {
                ...
                case '--generate-configuration':
                    ...
                    \file_put_contents('phpunit.xml', $generator->generateDefaultConfiguration(Version::series(), $bootstrapScript, $testsDirectory, $src));
    ...
    protected function handlePrinter($printerClass, $printerFile = '')
    {
        ...
        if (! $class->implementsInterface(TestListener::class)) {
            $this->exitWithErrorMessage(\sprintf('Could not use "%s" as printer: class does not implement %s', $printerClass, TestListener::class));
        }
        
        if (! $class->isSubclassOf(Printer::class)) {
            $this->exitWithErrorMessage(\sprintf('Could not use "%s" as printer: class does not extend %s', $printerClass, Printer::class));
        }
