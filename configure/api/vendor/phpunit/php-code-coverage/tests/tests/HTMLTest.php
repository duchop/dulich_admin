...
class HTMLTest extends TestCase
{
    ...
    protected function tearDown()
    {
        ...
        $tmpFilesIterator = new \RecursiveIteratorIterator(new \RecursiveDirectoryIterator(self::$TEST_TMP_PATH, \RecursiveDirectoryIterator::SKIP_DOTS), \RecursiveIteratorIterator::CHILD_FIRST);
    ...
    private function assertFilesEquals($expectedFilesPath, $actualFilesPath)
    {
        ...
        foreach ($expectedFilesIterator as $path => $fileInfo) {
            ...
            $this->assertStringMatchesFormatFile($fileInfo->getPathname(), str_replace(PHP_EOL, "\n", file_get_contents($actualFile)), "${filename} not match");
        }
    }
}
