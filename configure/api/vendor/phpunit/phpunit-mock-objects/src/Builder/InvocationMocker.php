...
class InvocationMocker implements MethodNameMatch
{
    ...
    public function willReturn($value, ...$nextValues)
    {
        if (\count($nextValues) === 0) {
            ...
            $stub = new Stub\ConsecutiveCalls(\array_merge([
                $value
            ], $nextValues));
        }
    ...
    public function method($constraint)
    {
        ...
        if (\is_string($constraint) && ! \in_array(\strtolower($constraint), $this->configurableMethods)) {
            throw new RuntimeException(\sprintf('Trying to configure method "%s" which cannot be configured because it does not exist, has not been specified, is final, or is static', $constraint));
        }
