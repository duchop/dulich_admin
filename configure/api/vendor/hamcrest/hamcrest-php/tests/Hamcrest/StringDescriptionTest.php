...
class StringDescriptionTest extends \PhpUnit_Framework_TestCase
{
    ...
    {
        $this->_description->appendValue(array(
            'foo',
            42.78
        ));
    ...
    {
        $this->_description->appendValueList('@start@', '@sep@ ', '@end@', array(
            'foo',
            42.78
        ));
    ...
    {
        $this->_description->appendDescriptionOf(new \Hamcrest\SampleSelfDescriber('foo'))->appendDescriptionOf(new \Hamcrest\SampleSelfDescriber('bar'));
        $this->assertEquals('foobar', (string) $this->_description);
    }
