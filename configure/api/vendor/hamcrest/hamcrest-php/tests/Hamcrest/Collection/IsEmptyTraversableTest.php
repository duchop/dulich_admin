...
class IsEmptyTraversableTest extends AbstractMatcherTest
{
    ...
    {
        $this->assertMatches(emptyTraversable(), new \ArrayObject(array()), 'an empty traversable');
    }
    ...
    {
        $this->assertDoesNotMatch(emptyTraversable(), null, 'should not match null');
    }
    ...
    {
        $this->assertDoesNotMatch(nonEmptyTraversable(), null, 'should not match null');
    }

    public function testNonEmptyDoesNotMatchWhenEmpty()
    {
        $this->assertDoesNotMatch(nonEmptyTraversable(), new \ArrayObject(array()), 'an empty traversable');
    }

    public function testNonEmptyMatchesWhenNotEmpty()
    {
        $this->assertMatches(nonEmptyTraversable(), new \ArrayObject(array(
            1,
            2,
            3
        )), 'a non-empty traversable');
