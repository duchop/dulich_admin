...
class PrettyPageHandler extends Handler
{
     ...
     * in an editor.
     * If the string contains the special substrings
        ...
        if (class_exists('Symfony\Component\VarDumper\Cloner\VarCloner')) {
            ...
            $cloner->addCasters([
                '*' => function ($obj, $a, $stub, $isNested, $filter = 0) {
                    ...
                    $classes = [
                        $class => $class
                    ] + class_parents($class) + class_implements($class);
     ...
     * resolver.
     * If the callable returns a string, it will
     * be set as the file reference's href attribute.
     *
     * @example $run->addEditor('macvim', "mvim://open?url=file://%file&line=%line")
     * @example $run->addEditor('remove-it', function($file, $line) {
     *          unlink($file);
     ...
     * return a string.
     *
     * @example $run->setEditor(function($file, $line) { return "file:///{$file}"; });
     * @example $run->setEditor('sublime');
     *         
     ...
     * act as an Ajax request.
     * The editor must be a
