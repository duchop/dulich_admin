...
trait MocksApplicationServices
{
    ...
    public function expectsEvents($events)
    {
        ...
        $this->beforeApplicationDestroyed(function () use($events) {
            ...
            $this->assertEmpty($eventsNotFired = array_diff($events, $fired), 'These expected events were not fired: [' . implode(', ', $eventsNotFired) . ']');
        });
    ...
    public function doesntExpectEvents($events)
    {
        ...
        $this->beforeApplicationDestroyed(function () use($events) {
            $this->assertEmpty($fired = $this->getFiredEvents($events), 'These unexpected events were fired: [' . implode(', ', $fired) . ']');
        });
    ...
    protected function expectsJobs($jobs)
    {
        ...
        $this->beforeApplicationDestroyed(function () use($jobs) {
            ...
            $this->assertEmpty($jobsNotDispatched = array_diff($jobs, $dispatched), 'These expected jobs were not dispatched: [' . implode(', ', $jobsNotDispatched) . ']');
        });
    ...
    protected function doesntExpectJobs($jobs)
    {
        ...
        $this->beforeApplicationDestroyed(function () use($jobs) {
            $this->assertEmpty($dispatched = $this->getDispatchedJobs($jobs), 'These unexpected jobs were dispatched: [' . implode(', ', $dispatched) . ']');
        });
    ...
    protected function withoutNotifications()
    {
        ...
        $mock->shouldReceive('send')->andReturnUsing(function ($notifiable, $instance, $channels = []) {
            $this->dispatchedNotifications[] = compact('notifiable', 'instance', 'channels');
        });
    ...
    protected function expectsNotification($notifiable, $notification)
    {
        ...
        $this->beforeApplicationDestroyed(function () use($notifiable, $notification) {
            foreach ($this->dispatchedNotifications as $dispatched) {
                ...
                if (($notified === $notifiable || $notified->getKey() == $notifiable->getKey()) && get_class($dispatched['instance']) === $notification) {
                    return $this;
                }
            }
