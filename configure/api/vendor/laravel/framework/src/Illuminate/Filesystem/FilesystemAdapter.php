...
class FilesystemAdapter implements FilesystemContract, CloudFilesystemContract
{
    ...
    {
        return (string) $adapter->getContainer()
            ->getObject($path)
            ->getPublicUrl();
    }
    ...
    public function getRackspaceTemporaryUrl($adapter, $path, $expiration, $options)
    {
        return $adapter->getContainer()
            ->getObject($path)
            ->getTemporaryUrl(Carbon::now()->diffInSeconds($expiration), $options['method'] ?? 'GET', $options['forcePublicUrl'] ?? true);
    }
    ...
    {
        return Collection::make($contents)->where('type', $type)
            ->pluck('path')
            ->values()
            ->all();
    }
