...
class Builder
{
    ...
    {
        list ($value, $operator) = $this->query->prepareValueAndOperator($value, $operator, func_num_args() == 2);
    ...
    {
        return $this->hydrate($this->query->getConnection()
    ...
    {
        return $this->model->hydrate($this->query->get($columns)
            ->all())
            ->all();
    }
    ...
    protected function addUpdatedAtColumn(array $values)
    {
        ...
        return Arr::add($values, $this->model->getUpdatedAtColumn(), $this->model->freshTimestampString());
    }
    ...
    {
        return $this->model->newInstance($attributes)->setConnection($this->query->getConnection()
            ->getName());
    }
    ...
    protected function parseWithRelations(array $relations)
    {
        ...
        foreach ($relations as $name => $constraints) {
            ...
            if (is_numeric($name)) {
                ...
                list ($name, $constraints) = Str::contains($name, ':') ? $this->createSelectWithConstraint($name) : [
                    $name,
                    function () {
                        //
                    }
