...
trait ValidatesAttributes
{
    ...
    public function validateDimensions($attribute, $value, $parameters)
    {
        ...
        if ($this->failsBasicDimensionChecks($parameters, $width, $height) || $this->failsRatioCheck($parameters, $width, $height)) {
            return false;
        }
    ...
    public function validateDistinct($attribute, $value, $parameters)
    {
        ...
        $attributeData = ValidationData::extractDataFromPath(ValidationData::getLeadingExplicitAttributePath($attributeName), $this->data);
    ...
    protected function getExistCount($connection, $table, $column, $value, $parameters)
    {
        ...
        return is_array($value) ? $verifier->getMultiCount($table, $column, $value, $extra) : $verifier->getCount($table, $column, $value, null, null, $extra);
    }
    ...
    protected function getUniqueIds($parameters)
    {
        ...
        return [
            $idColumn,
            $this->prepareUniqueId($parameters[2])
        ];
    }
    ...
    {
        return Str::contains($table, '.') ? explode('.', $table, 2) : [
            null,
            $table
        ];
    }
    ...
    {
        return isset($parameters[1]) && $parameters[1] !== 'NULL' ? $parameters[1] : $this->guessColumnForQuery($attribute);
    }
    ...
    protected function shouldBlockPhpUpload($value, $parameters)
    {
        ...
        return ($value instanceof UploadedFile) ? trim(strtolower($value->getClientOriginalExtension())) === 'php' : trim(strtolower($value->getExtension())) === 'php';
    }
