...
class PhoneNumber extends \Faker\Provider\PhoneNumber
{
     ...
     * 
     * @param string $type
     *            [def: landline] One of "landline" or "cellphone". Defaults to "landline" on invalid values.
     * @param bool $formatted
     *            [def: true] If the number should be formatted or not.
     * @return string
     */
    protected static function anyPhoneNumber($type, $formatted = true)
    {
        $area = static::areaCode();
        $number = ($type == 'cellphone') ? static::cellphone($formatted) : static::landline($formatted);
    ...
    {
        $method = static::randomElement(array(
            'cellphoneNumber',
            'landlineNumber'
        ));
