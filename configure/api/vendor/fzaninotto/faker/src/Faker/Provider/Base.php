...
class Base
{
     ...
     *
     * @param array $array
     *            Array to take elements from. Defaults to a-f
     * @param integer $count
    ...
    public static function regexify($regex = '')
    {
        ...
        $regex = preg_replace_callback('/\((.*?)\)/', function ($matches) {
            return Base::randomElement(explode('|', str_replace(array(
                '(',
                ')'
            ), '', $matches[1])));
        }, $regex);
     ...
     *
     * @param float|integer $weight
     *            Set the probability of receiving a null value.
     ...
     *            Maximum number of retries to find a unique value,
