...
class PersonTest extends TestCase
{
    ...
    {
        $cnp = $this->faker->cnp;
        $this->assertTrue($this->isValidCnp($cnp), sprintf("Invalid CNP '%' generated", $cnp));
    }
    ...
    public function test_validGender_returnsValidCnp()
    {
        ...
        $this->assertTrue($this->isValidMaleCnp($cnp), sprintf("Invalid CNP '%' generated for '%s' gender", $cnp, Person::GENDER_MALE));
        
        $cnp = $this->faker->cnp(Person::GENDER_FEMALE);
        $this->assertTrue($this->isValidFemaleCnp($cnp), sprintf("Invalid CNP '%' generated for '%s' gender", $cnp, Person::GENDER_FEMALE));
    }
    ...
    public function test_validYear_returnsValidCnp($value)
    {
        ...
        $this->assertTrue($this->isValidCnp($cnp), sprintf("Invalid CNP '%' generated for valid year '%s'", $cnp, $value));
    }
    ...
    public function test_nonResident_returnsValidCnp()
    {
        ...
        $this->assertTrue($this->isValidCnp($cnp), sprintf("Invalid CNP '%' generated for non resident", $cnp));
        $this->assertStringStartsWith('9', $cnp, sprintf("Invalid CNP '%' generated for non resident (should start with 9)", $cnp));
    }
     ...
     * @param
     *            $isResident
     * @param $expectedCnpStart @dataProvider
    ...
    public function test_validInputData_returnsValidCnp($gender, $dateOfBirth, $county, $isResident, $expectedCnpStart)
    {
        ...
        $this->assertStringStartsWith($expectedCnpStart, $cnp, sprintf("Invalid CNP '%' generated for non valid data", $cnp));
    }

    protected function isValidFemaleCnp($value)
    {
        return $this->isValidCnp($value) && in_array($value[0], array(
            2,
            4,
            6,
            8,
