    ...
    private function checkAttributes($stmts)
    {
        ...
        $traverser->addVisitor(new class() extends NodeVisitorAbstract {
            ...
            public function enterNode(Node $node)
            {
                ...
                if ($startLine < 0 || $endLine < 0 || $startFilePos < 0 || $endFilePos < 0 || $startTokenPos < 0 || $endTokenPos < 0) {
                    throw new \Exception('Missing location information on ' . $node->getType());
                }
                
                if ($endLine < $startLine || $endFilePos < $startFilePos || $endTokenPos < $startTokenPos) {
