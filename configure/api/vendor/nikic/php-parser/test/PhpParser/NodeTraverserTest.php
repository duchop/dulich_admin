...
class NodeTraverserTest extends TestCase
{
    ...
    public function testModifying()
    {
        ...
        $visitor1->expects($this->at(2))
            ->method('enterNode')
            ->with($str1Node)
            ->will($this->returnValue($str2Node));
        $visitor2->expects($this->at(2))
            ->method('enterNode')
            ->with($str2Node);
        
        // replace string2 node with string1 node again
        $visitor1->expects($this->at(3))
            ->method('leaveNode')
            ->with($str2Node)
            ->will($this->returnValue($str1Node));
        $visitor2->expects($this->at(3))
            ->method('leaveNode')
            ->with($str1Node);
        ...
        $visitor1->expects($this->at(4))
            ...
            ->with($printNode)
            ->will($this->returnValue($str1Node));
        $visitor2->expects($this->at(4))
            ->method('leaveNode')
            ->with($str1Node);
        
        // replace string1 node with empty statements again
        $visitor1->expects($this->at(5))
            ->method('afterTraverse')
            ->with([
            $str1Node
        ])
            ->will($this->returnValue([]));
        $visitor2->expects($this->at(5))
            ->method('afterTraverse')
            ->with([]);
    ...
    public function testDontTraverseChildren()
    {
        ...
        $visitor1->expects($this->at(6))
            ->method('leaveNode')
            ->with($negNode);
        $visitor2->expects($this->at(6))
            ->method('leaveNode')
            ->with($negNode);
    ...
    public function testInvalidReturn($visitor, $message)
    {
        ...
        $stmts = [
            new Node\Stmt\Expression(new Node\Scalar\LNumber(42))
        ];
    ...
    public function provideTestInvalidReturn()
    {
        ...
        return [
            [
                $visitor1,
                'enterNode() returned invalid value of type string'
            ],
            [
                $visitor2,
                'enterNode() returned invalid value of type string'
            ...
            [
                $visitor3,
                'leaveNode() returned invalid value of type string'
            ],
            [
                $visitor4,
                'leaveNode() returned invalid value of type string'
            ...
            [
                $visitor6,
                'bool(false) return from leaveNode() no longer supported. Return NodeTraverser::REMOVE_NODE instead'
            ],
            [
                $visitor7,
                'Trying to replace statement (Stmt_Expression) with expression (Scalar_LNumber). Are you missing a Stmt_Expression wrapper?'
            ],
            [
                $visitor8,
                'Trying to replace expression (Scalar_LNumber) with statement (Stmt_Return)'
            ]
        ];
    }
}
