...
class PrettyPrinterTest extends CodeTestAbstract
{
    ...
    public function testPrettyPrintExpr()
    {
        ...
        $expr = new Expr\Closure([
            'stmts' => [
                new Stmt\Return_(new String_("a\nb"))
            ]
        ]);
    ...
    public function testCommentBeforeInlineHTML()
    {
        ...
        $stmts = [
            new Stmt\InlineHTML('Hello World!', [
                'comments' => [
                    $comment
                ]
            ])
    ...
    public function provideTestKindAttributes()
    {
        ...
        return [
            ...
            [
                new String_("STRx", $nowdoc),
                "<<<'STR'\nSTRx\nSTR\n"
            ],
            [
                new String_("xSTR", $nowdoc),
                "<<<'STR'\nxSTR\nSTR\n"
            ],
    ...
    public function provideTestUnnaturalLiterals()
    {
        return [
            ...
            [
                new LNumber(- PHP_INT_MAX - 1),
                '(-' . PHP_INT_MAX . '-1)'
            ],
            [
                new LNumber(- 1, [
                    'kind' => LNumber::KIND_BIN
                ]),
                '-0b1'
            ],
            [
                new LNumber(- 1, [
                    'kind' => LNumber::KIND_OCT
                ]),
                '-01'
            ],
            [
                new LNumber(- 1, [
                    'kind' => LNumber::KIND_HEX
                ]),
                '-0x1'
            ],
            [
    ...
    {
        $stmts = [
            new Stmt\Expression(new Expr\PropertyFetch(new Expr\Variable('a'), new Expr\Error()))
        ];
        $prettyPrinter = new PrettyPrinter\Standard();
    ...
    {
        $stmts = [
            new Stmt\Expression(new Expr\ClassConstFetch(new Name('Foo'), new Expr\Error()))
        ];
        $prettyPrinter = new PrettyPrinter\Standard();
        $prettyPrinter->prettyPrint($stmts);
    }
...
CODE
    ...
    public function testRoundTripPrint($name, $code, $expected, $modeLine)
    {
         ...
         * the pretty printer tests (i.e.
         * returns the input if no changes occurred).
         */
    ...
    {
        return array_merge($this->getTests(__DIR__ . '/../code/prettyPrinter', 'test'), $this->getTests(__DIR__ . '/../code/parser', 'test'));
    }
}
