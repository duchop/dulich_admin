...
class ManifestSerializerTest extends \PHPUnit_Framework_TestCase
{
    ...
    public function testCanSerializeToString($expected)
    {
        ...
        $this->assertXmlStringEqualsXmlString($expected, $serializer->serializeToString($manifest));
    }
    ...
    public function dataProvider()
    {
        return [
            'application' => [
                file_get_contents(__DIR__ . '/_fixture/phpunit-5.6.5.xml')
            ],
            'library' => [
                file_get_contents(__DIR__ . '/_fixture/library.xml')
            ],
            'extension' => [
                file_get_contents(__DIR__ . '/_fixture/extension.xml')
            ]
        ];
    }
    ...
    public function testCanHandleUnknownType()
    {
        ...
        $manifest = new Manifest(new ApplicationName('testvendor/testname'), new Version('1.0.0'), $type, new CopyrightInformation(new AuthorCollection(), new License('bsd-3', new Url('https://some/uri'))), new RequirementCollection(), new BundledComponentCollection());
        
        $serializer = new ManifestSerializer();
        $this->assertXmlStringEqualsXmlFile(__DIR__ . '/_fixture/custom.xml', $serializer->serializeToString($manifest));
    }
}
