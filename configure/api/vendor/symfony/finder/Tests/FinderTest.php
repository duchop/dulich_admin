...
class FinderTest extends Iterator\RealIteratorTestCase
{
    ...
    public function testRemoveTrailingSlash()
    {
        ...
        $this->assertIterator($expected, $finder->in($in)
            ->files()
            ->getIterator());
    }
    ...
    public function testGetIterator()
    {
        ...
        $finder = $this->buildFinder();
        $this->assertEquals(2, iterator_count($finder->directories()
    ...
    public function testAppendWithAnArray()
    {
        ...
        $finder->append($this->toAbsolute(array(
            'foo',
            'toto'
        )));
    ...
    {
        $finder = $this->buildFinder();
        $finder->in(__DIR__ . \DIRECTORY_SEPARATOR . 'Fixtures' . \DIRECTORY_SEPARATOR . 'r+e.gex[c]a(r)s')->path('/^dir/');
        ...
        if (false === $couldRead = is_readable($testDir)) {
            try {
                ...
                if ($e instanceof \PHPUnit_Framework_ExpectationFailedException) {
                    $this->fail(sprintf("Expected exception:\n%s\nGot:\n%s\nWith comparison failure:\n%s", $expectedExceptionClass, 'PHPUnit_Framework_ExpectationFailedException', $e->getComparisonFailure()
                        ->getExpectedAsString()));
                }
                
                if ($e instanceof \PHPUnit\Framework\ExpectationFailedException) {
                    $this->fail(sprintf("Expected exception:\n%s\nGot:\n%s\nWith comparison failure:\n%s", $expectedExceptionClass, '\PHPUnit\Framework\ExpectationFailedException', $e->getComparisonFailure()
                        ->getExpectedAsString()));
                }
