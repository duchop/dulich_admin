...
class ErrorHandler
{
 ...
 // E_ERROR + E_CORE_ERROR + E_COMPILE_ERROR + E_PARSE
    ...
    public static function register(self $handler = null, $replace = true)
    {
        ...
        if (null === $prev = set_error_handler(array(
            $handler,
            'handleError'
        ))) {
        ...
        if (\is_array($prev = set_exception_handler(array(
            $handler,
            'handleException'
    ...
    public function handleError($type, $message, $file, $line)
    {
        ...
        if ($this->isRecursive) {
            ...
            try {
                ...
                $this->loggers[$type][0]->log($level, $logMessage, $errorAsException ? array(
                    'exception' => $errorAsException
                ) : array());
            } finally {
    ...
    public function handleException($exception, array $error = null)
    {
        ...
        if ($this->loggedErrors & $type) {
            try {
                $this->loggers[$type][0]->log($this->loggers[$type][1], $message, array(
                    'exception' => $exception
                ));
            } catch (\Exception $handlerException) {} catch (\Throwable $handlerException) {}
        }
