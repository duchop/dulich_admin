...
class ErrorHandlerTest extends TestCase
{
    ...
    public function testDefaultLogger()
    {
        try {
            ...
            $handler->setDefaultLogger($logger, array(
    ...
    public function testErrorStacking()
    {
        try {
            ...
            $logger->expects($this->exactly(2))
                ...
                $this->equalTo('User Warning: Silenced warning')
            ));
    ...
    public function testBootstrappingLogger()
    {
        ...
        $bootLogger->log(LogLevel::WARNING, 'Foo message', array(
            'exception' => $exception
        ));
        ...
        $mockLogger->expects($this->once())
            ->method('log')
            ->with(LogLevel::WARNING, 'Foo message', array(
            'exception' => $exception
        ));
        
        $handler->setLoggers(array(
            E_DEPRECATED => array(
                $mockLogger,
                LogLevel::WARNING
            )
    ...
    public function testSettingLoggerWhenExceptionIsBuffered()
    {
        ...
        $mockLogger->expects($this->once())
            ->method('log')
            ->with(LogLevel::CRITICAL, 'Uncaught Exception: Foo message', array(
            'exception' => $exception
        ));
    ...
    public function testHandleFatalErrorOnHHVM()
    {
        try {
            ...
            $logger->expects($this->once())
                ->method('log')
                ->with($this->equalTo(LogLevel::CRITICAL), $this->equalTo('Fatal Error: foo'));
