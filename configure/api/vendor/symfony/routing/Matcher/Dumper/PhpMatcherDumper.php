...
EOF;
     ...
     *
     * @param bool $supportsRedirections
...
EOF;
     ...
     *
     * @param StaticPrefixCollection $collection
     *            A StaticPrefixCollection instance
     * @param bool $supportsRedirections
     *            Whether redirections are supported by the base class
     * @param string $ifOrElseIf
     ...
     * @param bool $supportsRedirections
     *            Whether redirections are supported by the base class
     * @param string|null $parentPrefix
    ...
    private function compileRoute(Route $route, $name, $supportsRedirections, $parentPrefix = null)
    {
        ...
        if ($route->getCondition()) {
            $conditions[] = $this->getExpressionLanguage()->compile($route->getCondition(), array(
                'context',
                'request'
            ));
        }
...
EOF;
        ...
        if ($matches || $hostMatches) {
            ...
            $code .= sprintf("            \$ret = %s;\n", str_replace("\n", '', var_export(array_replace($route->getDefaults(), array(
                '_route' => $name
            )), true)));
        } else {
