...
class TraceableUrlMatcherTest extends TestCase
{
    ...
    public function test()
    {
        ...
        $coll->add('bar', new Route('/bar/{id}', array(), array(
            'id' => '\d+'
        )));
        $coll->add('bar1', new Route('/bar/{name}', array(), array(
            'id' => '\w+'
        ), array(), '', array(), array(
            'POST'
        )));
    ...
    public function testMatchRouteOnMultipleHosts()
    {
        ...
        $routes->add('first', new Route('/mypath/', array(
            '_controller' => 'MainBundle:Info:first'
        ), array(), array(), 'some.example.com'));
        
        $routes->add('second', new Route('/mypath/', array(
    ...
    public function testRoutesWithConditions()
    {
        ...
        $matchingRequest = Request::create('/foo', 'GET', array(), array(), array(), array(
