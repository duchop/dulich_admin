...
class RouteCollectionTest extends TestCase
{
    ...
    public function testRoute()
    {
        ...
        $this->assertEquals(array(
            'foo' => $route
        ), $collection->all(), '->add() adds a route');
    ...
    public function testOverriddenRoute()
    {
        ...
        $this->assertEquals('/foo1', $collection->get('foo')
            ->getPath());
    }
    ...
    public function testDeepOverriddenRoute()
    {
        ...
        $this->assertEquals('/foo2', $collection1->get('foo')
            ->getPath());
        $this->assertEquals('/foo2', $collection->get('foo')
            ->getPath());
    }
    ...
    public function testAddCollection()
    {
        ...
        $this->assertSame(array(
            ...
            'last' => $last
        ), $collection->all(), '->addCollection() imports routes of another collection, overrides if necessary and adds them at the end');
    }
    ...
    public function testAddCollectionWithResources()
    {
        ...
        $this->assertEquals(array(
            $foo,
            $foo1
        ), $collection->getResources(), '->addCollection() merges resources');
    }
    ...
    public function testAddDefaultsAndRequirementsAndOptions()
    {
        ...
        $collection1 = new RouteCollection();
        $collection1->add('bar', new Route('/{placeholder}', array(
            '_controller' => 'fixed',
            'placeholder' => 'default'
        ), array(
            'placeholder' => '.+'
    ...
    public function testAddPrefix()
    {
        ...
        $collection->addPrefix('/{admin}', array(
            'admin' => 'admin'
        ), array(
            'admin' => '\d+'
        ));
        $this->assertEquals('/{admin}/foo', $collection->get('foo')
            ->getPath(), '->addPrefix() adds a prefix to all routes');
        $this->assertEquals('/{admin}/bar', $collection->get('bar')
            ->getPath(), '->addPrefix() adds a prefix to all routes');
        $this->assertEquals(array(
            'admin' => 'admin'
        ), $collection->get('foo')
        ...
        $this->assertEquals('/0/{admin}/foo', $collection->get('foo')
            ->getPath(), '->addPrefix() ensures a prefix must start with a slash and must not end with a slash');
        $collection->addPrefix('/ /');
        $this->assertSame('/ /0/{admin}/foo', $collection->get('foo')
            ->getPath(), '->addPrefix() can handle spaces if desired');
        $this->assertSame('/ /0/{admin}/bar', $collection->get('bar')
    ...
    public function testAddPrefixOverridesDefaultsAndRequirements()
    {
        ...
        $collection->add('bar', $bar = new Route('/bar.{_format}', array(), array(
            '_format' => 'json'
        )));
        $collection->addPrefix('/admin', array(), array(
            '_format' => 'html'
        ));
        
        $this->assertEquals('html', $collection->get('foo')
            ->getRequirement('_format'), '->addPrefix() overrides existing requirements');
        $this->assertEquals('html', $collection->get('bar')
            ->getRequirement('_format'), '->addPrefix() overrides existing requirements');
    }
    ...
    public function testResource()
    {
        ...
        $this->assertEquals(array(
            $foo,
            $bar
        ), $collection->getResources(), '->addResource() adds a resource and getResources() only returns unique ones by comparing the string representation');
    }
    ...
    public function testRemove()
    {
        ...
        $this->assertSame(array(
            'bar' => $bar,
            'last' => $last
        ), $collection->all(), '->remove() can remove a single route');
    ...
    public function testClone()
    {
        ...
        $collection->add('b', new Route('/b', array(
            'placeholder' => 'default'
        ), array(
            'placeholder' => '.+'
        )));
    ...
    {
        $collection = new RouteCollection();
        $routea = new Route('/a', array(), array(), array(), '', array(), array(
            'GET',
            'POST'
        ));
        $routeb = new Route('/b');
