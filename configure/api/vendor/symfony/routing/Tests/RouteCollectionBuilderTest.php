...
class RouteCollectionBuilderTest extends TestCase
{
    ...
    {
        $routeCollectionBuilder = new RouteCollectionBuilder(new YamlFileLoader(new FileLocator(array(
            __DIR__ . '/Fixtures/'
        ))));
    ...
    public function testFlushSetsRouteNames()
    {
        ...
        $collectionBuilder->add('/blogs', 'AppBundle:Blog:list')->setMethods(array(
            'GET'
        ));
    ...
    public function testFlushSetsDetailsOnChildrenRoutes()
    {
        ...
        $this->assertEquals(array(
            'https'
        ), $actualListRoute->getSchemes());
        $this->assertEquals(array(
            'POST'
        ), $actualListRoute->getMethods());
        ...
        $this->assertEquals(array(
            'http'
        ), $actualEditRoute->getSchemes());
        $this->assertEquals(array(
            'GET',
            'POST'
        ), $actualEditRoute->getMethods());
    }
    ...
    public function testFlushPrefixesPaths($collectionPrefix, $routePath, $expectedPath)
    {
        ...
        $this->assertEquals($expectedPath, $collection->get('test_route')
            ->getPath());
    }
    ...
    public function testFlushSetsPrefixedWithMultipleLevels()
    {
        ...
        $collection = $routes->build();
        $this->assertEquals('/admin/dashboard', $collection->get('admin_dashboard')
            ->getPath(), 'Routes before mounting have the prefix');
        $this->assertEquals('/admin/users', $collection->get('admin_users')
            ->getPath(), 'Routes after mounting have the prefix');
        $this->assertEquals('/admin/blog/new', $collection->get('admin_blog_new')
            ->getPath(), 'Sub-collections receive prefix even if mounted before parent prefix');
        $this->assertEquals('/admin/stats/sales', $collection->get('admin_stats_sales')
            ->getPath(), 'Sub-collections receive prefix if mounted after parent prefix');
        $this->assertEquals('/admin/imported/foo', $collection->get('imported_route')
            ->getPath(), 'Normal RouteCollections are also prefixed properly');
    }
    ...
    public function testAutomaticRouteNamesDoNotConflict()
    {
        ...
        // route 2
        $accountRoutes->add('/dashboard', '')->setMethods(array(
            'GET'
        ));
        // route 3
        $accountRoutes->add('/dashboard', '')->setMethods(array(
