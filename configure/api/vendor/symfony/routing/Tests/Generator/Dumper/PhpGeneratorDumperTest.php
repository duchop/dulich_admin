...
class PhpGeneratorDumperTest extends TestCase
{
    ...
    public function testDumpWithRoutes()
    {
        ...
        $absoluteUrlWithParameter = $projectUrlGenerator->generate('Test', array(
            'foo' => 'bar'
        ), UrlGeneratorInterface::ABSOLUTE_URL);
        $absoluteUrlWithoutParameter = $projectUrlGenerator->generate('Test2', array(), UrlGeneratorInterface::ABSOLUTE_URL);
        $relativeUrlWithParameter = $projectUrlGenerator->generate('Test', array(
            'foo' => 'bar'
    ...
    {
        file_put_contents($this->testTmpFilepath, $this->generatorDumper->dump(array(
    ...
    {
        $this->routeCollection->add('Test', new Route('/testing/{foo}', array(
            'foo' => 'bar'
        )));
    ...
    {
        $this->routeCollection->add('Test1', new Route('/testing', array(), array(), array(), '', array(
            'ftp',
            'https'
        )));
