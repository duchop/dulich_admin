...
class ErrorListener implements EventSubscriberInterface
{
    ...
    public function onConsoleTerminate(ConsoleTerminateEvent $event)
    {
        ...
        if (! $inputString = $this->getInputString($event)) {
            return $this->logger->debug('The console exited with code "{code}"', array(
                'code' => $exitCode
            ));
        }
        
        $this->logger->debug('Command "{command}" exited with code "{code}"', array(
            'command' => $inputString,
            'code' => $exitCode
        ));
    }
