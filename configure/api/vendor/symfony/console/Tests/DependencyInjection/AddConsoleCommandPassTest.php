...
class AddConsoleCommandPassTest extends TestCase
{
    ...
    {
        $container = new ContainerBuilder();
        $command = $container->register('my-command', MyCommand::class)
            ->setPublic(false)
        ...
        $this->assertEquals(array(
            array(
                'my-command' => new ServiceClosureArgument(new TypedReference('my-command', MyCommand::class))
            )
        ), $commandLocator->getArguments());
        $this->assertSame(array(
            'console.command.symfony_component_console_tests_dependencyinjection_mycommand' => 'my-command'
        ), $container->getParameter('console.command.ids'));
        $this->assertSame(array(
            'my-command' => true
    ...
    {
        $container = new ContainerBuilder();
        $container->register('with-default-name', NamedCommand::class)
            ->setPublic(false)
        ...
        $this->assertEquals(array(
            array(
                'with-default-name' => new ServiceClosureArgument(new TypedReference('with-default-name', NamedCommand::class))
            )
        ), $commandLocator->getArguments());
        $this->assertSame(array(
            'console.command.symfony_component_console_tests_dependencyinjection_namedcommand' => 'with-default-name'
        ), $container->getParameter('console.command.ids'));
        $this->assertSame(array(
            'with-default-name' => true
        ), $container->getParameter('console.lazy_command.ids'));
