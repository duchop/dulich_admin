...
class ErrorListenerTest extends TestCase
{
    ...
    public function testOnConsoleError()
    {
        ...
        $listener->onConsoleError(new ConsoleErrorEvent(new ArgvInput(array(
            'console.php',
            'test:run',
            '--foo=baz',
            'buzz'
        )), $this->getOutput(), $error, new Command('test:run')));
    }
    ...
    public function testOnConsoleTerminateForNonZeroExitCodeWritesToLog()
    {
        ...
        $logger->expects($this->once())
            ->method('debug')
            ->with('Command "{command}" exited with code "{code}"', array(
            'command' => 'test:run',
            'code' => 255
        ));
        
        $listener = new ErrorListener($logger);
        $listener->onConsoleTerminate($this->getConsoleTerminateEvent(new ArgvInput(array(
            'console.php',
            'test:run'
        )), 255));
    }
    ...
    public function testAllKindsOfInputCanBeLogged()
    {
        ...
        $listener->onConsoleTerminate($this->getConsoleTerminateEvent(new ArrayInput(array(
            'name' => 'test:run',
            '--foo' => 'bar'
        )), 255));
    ...
    public function testCommandNameIsDisplayedForNonStringableInput()
    {
        ...
        $listener = new ErrorListener($logger);
        $listener->onConsoleTerminate($this->getConsoleTerminateEvent($this->getMockBuilder(InputInterface::class)
            ->getMock(), 255));
    }
