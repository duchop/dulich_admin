...
class LoggerDataCollectorTest extends TestCase
{
    ...
    {
        $logger = $this->getMockBuilder('Symfony\Component\HttpKernel\Log\DebugLoggerInterface')
            ->setMethods(array(
            'countErrors',
            'getLogs',
            'clear'
    ...
    public function testCollect($nb, $logs, $expectedLogs, $expectedDeprecationCount, $expectedScreamCount, $expectedPriorities = null)
    {
        ...
        $logs = array_map(function ($v) {
            if (isset($v['context']['exception'])) {
                ...
                $e = isset($e["\0*\0message"]) ? array(
                    $e["\0*\0message"],
                    $e["\0*\0severity"]
                ) : array(
                    $e["\0Symfony\Component\Debug\Exception\SilencedErrorContext\0severity"]
                );
            }
        ...
        yield 'logs with some deprecations' => array(
            ...
            array(
                array(
                    'message' => 'foo3',
                    'context' => array(
                        'exception' => new \ErrorException('warning', 0, E_USER_WARNING)
                    ),
                    'priority' => 100,
                    'priorityName' => 'DEBUG'
                ),
                array(
                    'message' => 'foo',
                    'context' => array(
                        'exception' => new \ErrorException('deprecated', 0, E_DEPRECATED)
                    ),
                    'priority' => 100,
                    'priorityName' => 'DEBUG'
                ),
        ...
        yield 'logs with some silent errors' => array(
            ...
            array(
                ...
                array(
                    'message' => 'foo3',
                    'context' => array(
                        'exception' => new SilencedErrorContext(E_USER_WARNING, __FILE__, __LINE__)
                    ),
                    'priority' => 100,
                    'priorityName' => 'DEBUG'
                )
