...
class ControllerResolverTest extends TestCase
{
    ...
    public function testGetControllerWithObjectAndMethod()
    {
        ...
        $request = Request::create('/');
        $request->attributes->set('_controller', array(
            $this,
            'controllerMethod1'
        ));
        $controller = $resolver->getController($request);
    ...
    public function getUndefinedControllers()
    {
        return array(
            array(
                ...
                'Unable to find controller "1".'
            ),
            array(
                'foo',
                'InvalidArgumentException',
                'Unable to find controller "foo".'
            ),
            array(
                'oof::bar',
                'InvalidArgumentException',
                'Class "oof" does not exist.'
            ),
            array(
                'stdClass',
                'InvalidArgumentException',
                'Unable to find controller "stdClass".'
            ),
            array(
    ...
    public function testGetArguments()
    {
        ...
        $this->assertEquals(array(
            'foo'
        ), $resolver->getArguments($request, $controller), '->getArguments() returns an array of arguments for the controller method');
        ...
        $this->assertEquals(array(
            'foo',
            null
        ), $resolver->getArguments($request, $controller), '->getArguments() uses default values if present');
        
        $request->attributes->set('bar', 'bar');
        $this->assertEquals(array(
            'foo',
            'bar'
        ), $resolver->getArguments($request, $controller), '->getArguments() overrides default values if provided in the request attributes');
        ...
        $this->assertEquals(array(
            $request
        ), $resolver->getArguments($request, $controller), '->getArguments() injects the request');
    }
    ...
    public function testGetVariadicArguments()
    {
        ...
        $request = Request::create('/');
        $request->attributes->set('foo', 'foo');
        $request->attributes->set('bar', array(
            'foo',
            'bar'
        ));
        $controller = array(
    ...
    {
        $mock = $this->getMockBuilder('Symfony\Component\HttpKernel\Controller\ControllerResolver')
            ->setMethods(array(
            'createController'
        ))
            ->getMock();
        $mock->expects($this->once())
            ...
            ->will($this->returnValue('Symfony\Component\HttpKernel\Tests\Controller\some_controller_function'));
