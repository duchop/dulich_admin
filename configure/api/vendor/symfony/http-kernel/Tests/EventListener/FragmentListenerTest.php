...
class FragmentListenerTest extends TestCase
{
    ...
    public function testWithSignature()
    {
        ...
        $request = Request::create($signer->sign('http://example.com/_fragment?_path=foo%3Dbar%26_controller%3Dfoo'), 'GET', array(), array(), array(), array(
            'REMOTE_ADDR' => '10.0.0.1'
        ));
    ...
    {
        $signer = new UriSigner('foo');
        $request = Request::create($signer->sign('http://example.com/_fragment?_path=foo%3Dbar'), 'GET', array(), array(), array(), array(
            'REMOTE_ADDR' => '10.0.0.1'
        ));
