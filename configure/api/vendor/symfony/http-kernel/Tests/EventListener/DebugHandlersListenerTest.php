...
class DebugHandlersListenerTest extends TestCase
{
    ...
    public function testConfigure()
    {
        ...
        $eHandler->setExceptionHandler(array(
            $xHandler,
            'handle'
        ));
    ...
    public function testConfigureForHttpKernelWithNoTerminateWithException()
    {
        ...
        $eHandler = new ErrorHandler();
        $event = new KernelEvent($this->getMockBuilder('Symfony\Component\HttpKernel\HttpKernelInterface')->getMock(), Request::create('/'), HttpKernelInterface::MASTER_REQUEST);
        
        $exception = null;
        $h = set_exception_handler(array(
            $eHandler,
            'handleException'
        ));
        try {
