...
class FileLinkFormatter implements \Serializable
{
    ...
    /**
     *
     * @param string|\Closure $urlFormat
    ...
    public function __construct($fileLinkFormat = null, RequestStack $requestStack = null, $baseDir = null, $urlFormat = null)
    {
        ...
        if ($fileLinkFormat && ! \is_array($fileLinkFormat)) {
            ...
            $fileLinkFormat = array(
                substr($f, 0, $i)
            ) + preg_split('/&([^>]++)>/', substr($f, $i), - 1, PREG_SPLIT_DELIM_CAPTURE);
        }
    ...
    public function unserialize($serialized)
    {
        if (\PHP_VERSION_ID >= 70000) {
            $this->fileLinkFormat = unserialize($serialized, array(
