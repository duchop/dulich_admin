...
class PdoSessionHandlerTest extends TestCase
{
    ...
    {
        $storage = new PdoSessionHandler($this->getMemorySqlitePdo(), array(
    ...
    public function testReadConvertsStreamToString()
    {
        ...
        $pdo->prepareResult->expects($this->once())
            ->method('fetchAll')
    ...
    public function testSessionDestroy()
    {
        ...
        $storage->write('id', 'data');
        $storage->close();
        $this->assertEquals(1, $pdo->query('SELECT COUNT(*) FROM sessions')
            ->fetchColumn());
        ...
        $storage->destroy('id');
        $storage->close();
        $this->assertEquals(0, $pdo->query('SELECT COUNT(*) FROM sessions')
            ->fetchColumn());
    ...
    public function testSessionGC()
    {
        ...
        $storage->close();
        $this->assertEquals(2, $pdo->query('SELECT COUNT(*) FROM sessions')
    ...
    public function provideUrlDsnPairs()
    {
        ...
        yield 'sqlite relative path' => array(
            'sqlite://localhost/tmp/test',
            'sqlite:tmp/test'
        );
        yield 'sqlite absolute path' => array(
            'sqlite://localhost//tmp/test',
            'sqlite:/tmp/test'
        );
        yield 'sqlite relative path without host' => array(
            'sqlite:///tmp/test',
            'sqlite:tmp/test'
        );
        yield 'sqlite absolute path without host' => array(
            'sqlite3:////tmp/test',
            'sqlite:/tmp/test'
        );
        yield array(
