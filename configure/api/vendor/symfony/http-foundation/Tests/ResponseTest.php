...
class ResponseTest extends ResponseTestCase
{
    ...
    public function testGetMaxAge()
    {
        ...
        $response = new Response();
        $response->headers->set('Cache-Control', 'must-revalidate');
        $response->headers->set('Expires', $this->createDateTimeOneHourLater()
            ->format(DATE_RFC2822));
        ...
        $this->assertEquals('Sat, 01 Jan 00 00:00:00 +0000', $response->getExpires()
            ->format(DATE_RFC822));
    ...
    public function testGetTtl()
    {
        ...
        $response = new Response();
        $response->headers->set('Expires', $this->createDateTimeOneHourAgo()
            ->format(DATE_RFC2822));
        $this->assertLessThan(0, $response->getTtl(), '->getTtl() returns negative values when Expires is in past');
        
        $response = new Response();
        $response->headers->set('Expires', $response->getDate()
            ->format(DATE_RFC2822));
    ...
    public function testGetVary()
    {
        ...
        $this->assertEquals(array(
            'Accept-Language'
        ), $response->getVary(), '->getVary() parses a single header name value');
        ...
        $this->assertEquals(array(
            ...
            'User-Agent',
            'X-Foo'
        ), $response->getVary(), '->getVary() parses multiple header name values separated by spaces');
        ...
        $this->assertEquals(array(
            ...
            'User-Agent',
            'X-Foo'
        ), $response->getVary(), '->getVary() parses multiple header name values separated by commas');
    ...
    public function testSetVary()
    {
        ...
        $this->assertEquals(array(
            'Accept-Language',
            'User-Agent'
        ), $response->getVary(), '->setVary() replace the vary header by default');
        ...
        $this->assertEquals(array(
            ...
            'User-Agent',
            'X-Foo'
        ), $response->getVary(), '->setVary() doesn\'t wipe out earlier Vary headers if replace is set to false');
    }

    public function testDefaultContentType()
    {
        $headerMock = $this->getMockBuilder('Symfony\Component\HttpFoundation\ResponseHeaderBag')
            ->setMethods(array(
            'set'
        ))
            ->getMock();
    ...
    public function testSetCache()
    {
        ...
        $response->setCache($options);
        $this->assertEquals($response->getLastModified()
    ...
    public function testSetExpires()
    {
        ...
        $this->assertEquals($response->getExpires()
            ->getTimestamp(), $now->getTimestamp());
    }
