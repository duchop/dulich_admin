...
class RegisterListenersPassTest extends TestCase
{
        ...
        $expectedCalls = array(
            array(
                ...
                array(
                    'event',
                    array(
                        new ServiceClosureArgument(new Reference('my_event_subscriber')),
                        'onEvent'
                    ),
                    0
                )
    ...
    public function testHotPathEvents()
    {
        ...
        (new RegisterListenersPass())->setHotPathEvents(array(
            'event'
        ))->process($container);
        
        $this->assertTrue($container->getDefinition('foo')
            ->hasTag('container.hot_path'));
    }
    ...
    {
        $container = new ContainerBuilder();
        $container->register('foo', \stdClass::class)->addTag('kernel.event_listener', array(
            'event' => 'foo.bar'
        ));
        $container->register('bar', InvokableListenerService::class)->addTag('kernel.event_listener', array(
            'event' => 'foo.bar'
        ));
        $container->register('baz', InvokableListenerService::class)->addTag('kernel.event_listener', array(
            'event' => 'event'
        ));
