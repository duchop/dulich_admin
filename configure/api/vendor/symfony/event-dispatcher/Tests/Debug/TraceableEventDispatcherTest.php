...
class TraceableEventDispatcherTest extends TestCase
{
    ...
    public function testLogger()
    {
        ...
        $logger->expects($this->at(0))
            ->method('debug')
            ->with('Notified event "{event}" to listener "{listener}".', array(
            'event' => 'foo',
            'listener' => 'closure'
        ));
    ...
    public function testLoggerWithStoppedEvent()
    {
        ...
        $logger->expects($this->at(1))
            ->method('debug')
            ->with('Listener "{listener}" stopped propagation of the event "{event}".', array(
            'event' => 'foo',
            'listener' => 'closure'
        ));
        $logger->expects($this->at(2))
            ->method('debug')
            ->with('Listener "{listener}" was not called for event "{event}".', array(
            'event' => 'foo',
            'listener' => 'closure'
        ));
