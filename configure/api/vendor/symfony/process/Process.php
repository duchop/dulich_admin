...
class Process implements \IteratorAggregate
{
 ...
 // Use this flag to skip STDERR while iterating
     ...
     * @param array $env
     *            An array of additional env vars to set when running the process
     *            
    ...
    public function start(callable $callback = null/*, array $env = array()*/)
    {
        ...
        if (\is_array($commandline = $this->commandline)) {
            $commandline = implode(' ', array_map(array(
                $this,
                'escapeArgument'
            ), $commandline));
    ...
    {
        return \is_array($this->commandline) ? implode(' ', array_map(array(
            $this,
            'escapeArgument'
    ...
    public function setTty($tty)
    {
        ...
        if ($tty) {
            ...
            if (null === $isTtySupported) {
                $isTtySupported = (bool) @proc_open('echo 1 >/dev/null', array(
                    array(
                        'file',
                        '/dev/tty',
                        'r'
    ...
    public static function isPtySupported()
    {
        ...
        return $result = (bool) @proc_open('echo 1 >/dev/null', array(
            array(
                'pty'
            ),
            array(
    ...
    private function doSignal($signal, $throwException)
    {
        ...
        if ('\\' === \DIRECTORY_SEPARATOR) {
            ...
            } elseif ($ok = proc_open(sprintf('kill -%d %d', $signal, $pid), array(
                2 => array(
                    'pipe',
                    'w'
                )
