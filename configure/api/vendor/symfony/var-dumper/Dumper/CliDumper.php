...
class CliDumper extends AbstractDumper
{
    ...
    public function enterHash(Cursor $cursor, $type, $class, $hasChild)
    {
        ...
        if ($cursor->softRefCount || 0 < $cursor->softRefHandle) {
            $prefix .= $this->style('ref', (Cursor::HASH_RESOURCE === $type ? '@' : '#') . (0 < $cursor->softRefHandle ? $cursor->softRefHandle : $cursor->softRefTo), array(
                'count' => $cursor->softRefCount
            ));
        } elseif ($cursor->hardRefTo && ! $cursor->refIndex && $class) {
            $prefix .= $this->style('ref', '&' . $cursor->hardRefTo, array(
    ...
    protected function dumpKey(Cursor $cursor)
    {
        if (null !== $key = $cursor->hashKey) {
            ...
            switch ($cursor->hashType) {
                ...
                case Cursor::HASH_OBJECT:
                    if (! isset($key[0]) || "\0" !== $key[0]) {
                        ...
                        $this->line .= '-' . $bin . '"' . $this->style('private', $key, array(
                            'class' => ''
                        )) . '": ';
                    }
                    break;
            ...
            if ($cursor->hardRefTo) {
                $this->line .= $this->style('ref', '&' . ($cursor->hardRefCount ? $cursor->hardRefTo : ''), array(
                    'count' => $cursor->hardRefCount
                )) . ' ';
            }
        }
    ...
    protected function supportsColors()
    {
        ...
        $h = stream_get_meta_data($this->outputStream) + array(
            'wrapper_type' => null
        );
