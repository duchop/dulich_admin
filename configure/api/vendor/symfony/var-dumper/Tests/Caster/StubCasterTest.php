...
EODUMP;
    ...
    {
        $var = array(
            new LinkStub(__CLASS__, 0, __FILE__)
        );
...
EODUMP;
    ...
    {
        $var = array(
            new LinkStub('example.com', 0, 'http://example.com')
        );
...
EODUMP;
    ...
    public function testClassStub()
    {
        ...
        $dumper->setDumpHeader('<foo></foo>');
        $dumper->setDumpBoundaries('<bar>', '</bar>');
        $dump = $dumper->dump($cloner->cloneVar($var), true, array(
            'fileLinkFormat' => '%f:%l'
        ));
...
EODUMP;
    ...
    {
        $var = array(
            new ClassStub(NotExisting::class)
        );
