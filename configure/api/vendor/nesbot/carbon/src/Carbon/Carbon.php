...
class Carbon extends DateTime implements JsonSerializable
{
    ...
    protected static $regexFormats = array(
        ...
        'r' => '(([a-zA-Z]{3}), ([123][0-9]|[1-9]) ([a-zA-Z]{3}) ([1-9][0-9]{0,4}) (2[0-3]|[01][0-9]):([0-5][0-9]):([0-5][0-9]) [\+\-](1[012]|0[0-9])([0134][05]))'
    ) // D, j M Y H:i:s O
;
    ...
    /**
     * Add microseconds to now on PHP < 7.1 and 7.1.3.
     * true by default.
     *
     * @var bool
     */
    ...
    public function __construct($time = null, $tz = null)
    {
        ...
        // @codeCoverageIgnoreStart
        if ($isNow && ! isset($testInstance) && static::isMicrosecondsFallbackEnabled() && (version_compare(PHP_VERSION, '7.1.0-dev', '<') || version_compare(PHP_VERSION, '7.1.3-dev', '>=') && version_compare(PHP_VERSION, '7.1.4-dev', '<'))) {
    ...
    /**
     * Create a Carbon instance from just a date.
     * The time portion is set to midnight.
     *
     * @param int|null $year            
     * @param int|null $month            
     * @param int|null $day            
    ...
    {
        return $tz !== null ? parent::createFromFormat($format, $time, static::safeCreateDateTimeZone($tz)) : parent::createFromFormat($format, $time);
    }
    ...
    protected static function expectDateTime($date, $other = array())
    {
        ...
        if (! $date instanceof DateTime && ! $date instanceof DateTimeInterface) {
            throw new InvalidArgumentException($message . 'DateTime or DateTimeInterface, ' . (is_object($date) ? get_class($date) : gettype($date)) . ' given');
        }
    }
    ...
    protected function resolveCarbon($date = null)
    {
        ...
        static::expectDateTime($date, array(
            'null',
            'string'
        ));
    ...
    public function __get($name)
    {
        ...
        switch (true) {
            ...
            case $name === 'local':
                return $this->getOffset() === $this->copy()
     ...
     * instance is created.
     * The provided instance will be returned
    ...
    /**
     * Determine if there is a valid test instance set.
     * A valid test instance
    ...
    public static function localeHasShortUnits($locale)
    {
        return static::executeWithLocale($locale, function ($newLocale, TranslatorInterface $translator) {
            return $newLocale && (($y = $translator->trans('y')) !== 'y' && $y !== $translator->trans('year')) || (($y = $translator->trans('d')) !== 'd' && $y !== $translator->trans('day')) || (($y = $translator->trans('h')) !== 'h' && $y !== $translator->trans('hour'));
        });
    }
    ...
    {
        return static::executeWithLocale($locale, function ($newLocale, TranslatorInterface $translator) {
            return $newLocale && $translator->trans('ago') !== 'ago' && $translator->trans('from_now') !== 'from_now' && $translator->trans('before') !== 'before' && $translator->trans('after') !== 'after';
        });
    }
    ...
    {
        return $checkMicroseconds ? $this->format('H:i:s.u') === '00:00:00.000000' : $this->format('H:i:s') === '00:00:00';
    }
    ...
    {
        return $checkMicroseconds ? $this->format('H:i:s.u') === '23:59:59.999999' : $this->format('H:i:s') === '23:59:59';
    }
     ...
     * Positive $value travels forward while
    ...
    /**
     * Add months without overflowing to the instance.
     * Positive $value
    ...
    /**
     * Add hours to the instance using timestamp.
     * Positive $value travels
    ...
    /**
     * Add minutes to the instance using timestamp.
     * Positive $value
     * travels forward while negative $value travels into the past.
     *
     * @param int $value            
    ...
    {
        return (int) $this->diff($this->resolveCarbon($date), $absolute)
            ->format('%r%y');
    }
    ...
    public function diffInSeconds($date = null, $absolute = true)
    {
        ...
        $value = $diff->days * static::HOURS_PER_DAY * static::MINUTES_PER_HOUR * static::SECONDS_PER_MINUTE + $diff->h * static::MINUTES_PER_HOUR * static::SECONDS_PER_MINUTE + $diff->i * static::SECONDS_PER_MINUTE + $diff->s;
    ...
    {
        return $this->diffInSeconds($this->copy()
            ->startOfDay());
    }
    ...
    public function diffForHumans($other = null, $absolute = false, $short = false, $parts = 1)
    {
        ...
        $diffIntervalArray = array(
            array(
                'value' => $diffInterval->y,
                'unit' => 'year',
                'unitShort' => 'y'
            ),
            array(
                'value' => $diffInterval->m,
                'unit' => 'month',
                'unitShort' => 'm'
            ),
            array(
                'value' => $diffInterval->d,
                'unit' => 'day',
                'unitShort' => 'd'
            ),
            array(
                'value' => $diffInterval->h,
                'unit' => 'hour',
                'unitShort' => 'h'
            ),
            array(
                'value' => $diffInterval->i,
                'unit' => 'minute',
                'unitShort' => 'min'
            ),
            array(
                'value' => $diffInterval->s,
                'unit' => 'second',
                'unitShort' => 's'
            )
        );
        ...
        foreach ($diffIntervalArray as $diffIntervalData) {
            if ($diffIntervalData['value'] > 0) {
                ...
                if ($diffIntervalData['unit'] === 'day' && $count >= static::DAYS_PER_WEEK) {
                    ...
                    $interval[] = static::translator()->transChoice($unit, $count, array(
                        ':count' => $count
                    ));
        ...
        if ($parts === 1) {
            ...
            if ($key !== static::translator()->transChoice($key, $count)) {
                $time = static::translator()->transChoice($key, $count, array(
                    ':count' => $count
                ));
            }
        }
        
        return static::translator()->trans($transId, array(
            ':time' => $time
        ));
    }
     ...
     * If no dayOfWeek is provided, modify to the
     ...
     * in the current month.
     * If the calculated occurrence is outside the scope
