...
class CarbonInterval extends DateInterval
{
    ...
    private static function getFlipCascadeFactors()
    {
        if (! self::$flipCascadeFactors) {
            ...
            foreach (static::getCascadeFactors() as $to => $tuple) {
                ...
                self::$flipCascadeFactors[self::standardizeUnit($from)] = array(
            ...
            switch (strtolower($unit)) {
                ...
                case 'h':
                    ...
                    if ($fraction) {
                        $parts[] = array(
                            null,
                            $fraction * static::getMinutesPerHours(),
                            'm'
                        );
                    }
                    break;
                ...
                default:
                    throw new InvalidArgumentException(sprintf('Invalid part %s in definition %s', $part, $intervalDefinition));
            }
        }
    ...
    public function forHumans($short = false)
    {
        ...
        foreach ($periods as $unit => $options) {
            ...
            if ($count > 0) {
                $parts[] = static::translator()->transChoice($short ? $shortUnit : $unit, $count, array(
                    ':count' => $count
                ));
            }
        }
    ...
    {
        return CarbonPeriod::createFromArray(array_merge(array(
            $this
        ), func_get_args()));
    }
