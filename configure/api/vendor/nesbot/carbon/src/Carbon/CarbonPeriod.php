...
class CarbonPeriod implements Iterator, Countable
{
    ...
    /**
     * Underlying date interval instance.
     * Always present, one day by default.
     *
     * @var CarbonInterval
     */
    ...
    /**
     * Period start date.
     * Applied on rewind. Always present, now by default.
     *
     * @var Carbon
     */
    protected $startDate;
    ...
    /**
     * Period end date.
     * For inverted interval should be before the start date. Applied via a filter.
     *
     * @var Carbon|null
     */
    protected $endDate;
    ...
    /**
     * Limit for number of recurrences.
     * Applied via a filter.
     *
     * @var int|null
     */
    ...
    /**
     * Index of current date.
     * Always sequential, even if some dates are skipped by filters.
    ...
    /**
     * Current date.
     * May temporarily hold unaccepted value when looking for a next valid date.
     * Equal to null only before the first iteration.
     *
     * @var Carbon
     */
    ...
    public function toString()
    {
        ...
        if ($this->recurrences !== null) {
            $parts[] = $translator->transChoice('period_recurrences', $this->recurrences, array(
                ':count' => $this->recurrences
            ));
        }
        
        $parts[] = $translator->trans('period_interval', array(
            ':interval' => $this->dateInterval->forHumans()
        ));
        
        $parts[] = $translator->trans('period_start_date', array(
            ':date' => $this->startDate->format($format)
        ));
        
        if ($this->endDate !== null) {
            $parts[] = $translator->trans('period_end_date', array(
                ':date' => $this->endDate->format($format)
            ));
        }
        ...
        switch ($method) {
            ...
            case 'second':
                return $this->setDateInterval(call_user_func(
                    ...
                    array(
                        $this->isDefaultInterval ? new CarbonInterval('PT0S') : $this->dateInterval,
                        $method
